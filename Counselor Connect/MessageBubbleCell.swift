...
class MessageBubbleCell: UITableViewCell {
    ...
    override init(style: UITableViewCellStyle, reuseIdentifier: String?) {
        ...
        bubbleImageView.userInteractionEnabled = false // #CopyMesage
        
        messageLabel = UILabel(frame: CGRect(x: 0, y: 0, width: 218, height: 0))
        messageLabel.font = UIFont.systemFontOfSize(messageFontSize)
        messageLabel.numberOfLines = 0
        messageLabel.userInteractionEnabled = false   // #CopyMessage

        messageTextView = UITextView(frame: CGRect(x: 0, y: 0, width: 218, height: 100))
        messageTextView.font = UIFont.systemFontOfSize(messageFontSize)
    ...
    func configureWithMessage(message: Message) {
        ...
        let testLabel = UILabel(frame: CGRect(x: 0, y: 0, width: 218, height: 0))
        testLabel.font = UIFont.systemFontOfSize(messageFontSize)
        testLabel.lineBreakMode = NSLineBreakMode.ByWordWrapping
        testLabel.adjustsFontSizeToFitWidth = false
        testLabel.numberOfLines = 0
        testLabel.text = message.text
        testLabel.sizeToFit()
        cellHeight = testLabel.frame.height + 24
...
func bubbleImageMake() -> (incoming: UIImage, incomingHighlighed: UIImage, outgoing: UIImage, outgoingHighlighed: UIImage) {
    ...
    let incoming = coloredImage(maskIncoming, 150/255.0, 230/255.0, 250/255.0, 1).resizableImageWithCapInsets(capInsetsIncoming)
    ...
    let outgoing = coloredImage(maskOutgoing, 150/255.0, 180/255.0, 240/255.0, 1).resizableImageWithCapInsets(capInsetsOutgoing)
    let outgoingHighlighted = coloredImage(maskOutgoing, 140/255.0, 190/255.0, 200/255.0, 1).resizableImageWithCapInsets(capInsetsOutgoing)
